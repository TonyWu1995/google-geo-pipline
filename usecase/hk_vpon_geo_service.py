...
class HKVponGeoService:
    ...
    def generate(self, hk_geo_list: list):
        ...
        for hk_geo in hk_geo_list:
            vpon_id_list = self.calc_vpon_id(hk_mapping_dict, hk_geo)
            result.append(VponGeoDomain.build(hk_geo[0], vpon_id_list[0], vpon_id_list[1], vpon_id_list[2]))
        log.debug("generate() result size={} hk_mapping_dict size={}".format(len(result), len(hk_mapping_dict)))
        return result

    def calc_vpon_id(self, hk_mapping_dict, hk_geo):
        log.debug("match() hk_geo={}".format(hk_geo))
